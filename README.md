...
2 - Realizar um git clone do projeto para baixar em sua máquina, através do comando:
  ```javascript
  git clone https://github.com/LucasAquino21/APS_CalculoCarbono.git 
  ```
  
3 - Abrir o prompt de comando Windows ou Linux e navegar até a pasta do projeto. (........\APS_PegadaCarbono) e baixar os módulos no node, através do comando:
   ```javascript
   npm install
   ```
...
5.1 - Faremos a requisição ao nosso servidor local utilizando o método POST através do endereço "http://localhost:12000/pegadaCarbono".
